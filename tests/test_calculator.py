...
def test_save_history_failure(calculator, caplog):
    ...
    with patch('app.calculator.pd.DataFrame.to_csv', side_effect=IOError(error_message)):
        with pytest.raises(OperationError, match=f"Failed to save history: {error_message}"):
            calculator.save_history()
        assert f"Failed to save history: {error_message}" in caplog.text
...
def test_load_history_failure(calculator, caplog):
    ...
    with patch('app.calculator.pd.read_csv', side_effect=ValueError(error_message)):
        with pytest.raises(OperationError, match=f"Failed to load history: {error_message}"):
            calculator.load_history()
        assert f"Failed to load history: {error_message}" in caplog.text


def test_get_history_dataframe(calculator):
    """Test getting the history as a pandas DataFrame."""
    # 1. Test with an empty history
    df_empty = calculator.get_history_dataframe()
    assert isinstance(df_empty, pd.DataFrame), "Should return a DataFrame even if empty"
    assert df_empty.empty, "DataFrame should be empty when history is empty"

    # 2. Test with a populated history
    operation = OperationFactory.create_operation('add')
    calculator.set_operation(operation)
    calculator.perform_operation(15, 10)

    df_populated = calculator.get_history_dataframe()
    assert len(df_populated) == 1, "DataFrame should have one row after one calculation"
    
    # Verify the content of the DataFrame row
    expected_columns = ['operation', 'operand1', 'operand2', 'result', 'timestamp']
    assert list(df_populated.columns) == expected_columns, "DataFrame should have the correct columns"
    
    first_row = df_populated.iloc[0]
    assert first_row['result'] == '25'
    assert first_row['operation'] == 'Addition'
...
def test_calculator_repl_exit_save_failure(mock_print, mock_input):
    ...
    with patch('app.calculator.Calculator.save_history', side_effect=Exception(error_message)):
        # Act: Run the REPL
        calculator_repl()
        # Assert: Check that the warning and goodbye messages are printed
        mock_print.assert_any_call(f"Warning: Could not save history: {error_message}")
        mock_print.assert_any_call("Goodbye!")

@patch('builtins.input', side_effect=['history', 'add', '1', '2', 'history', 'exit'])
@patch('builtins.print')
def test_calculator_repl_history(mock_print, mock_input):
    """Test the 'history' command in the REPL for both empty and populated states."""
    with TemporaryDirectory() as temp_dir:
        config = CalculatorConfig(base_dir=Path(temp_dir), max_history_size=2)

        # Ensure there's no stale history file
        try:
            if config.history_file.exists():
                config.history_file.unlink()
        except Exception:
        ...
        # Factory that forces the Calculator used by the REPL to be built with our config
        def calculator_factory(*args, **kwargs):
            return RealCalculator(config=config)

        # Patch the Calculator class *inside* the app.calculator module so when
        # calculator_repl() calls Calculator() it receives our pre-configured instance.
        with patch('app.calculator.Calculator', side_effect=calculator_factory):
            # Run the REPL while our temp dir is still active
            calculator_repl()

    # Verify the output for an empty history (first 'history')
    mock_print.assert_any_call("No calculations in history")
    # Verify the output for a populated history (after the add)
...
def test_calculator_repl_save_success(mock_print, mock_input):
    ...
    with patch('app.calculator.Calculator.save_history') as mock_save:
        calculator_repl()
        # The REPL will call save once for the explicit 'save' command and
        # once again when handling the subsequent 'exit' command which also
        # attempts to save before quitting. Assert both calls occurred.
...
def test_calculator_repl_fatal_error_on_init(capsys, caplog):
    """Test that a fatal error during Calculator initialization is handled."""
    error_message = "Initialization failed"
    # Patch the entire Calculator class to raise an exception on instantiation
    with patch('app.calculator_repl.Calculator', side_effect=Exception(error_message)):
        with pytest.raises(Exception, match=error_message):
            with caplog.at_level(logging.ERROR):
                calculator_repl()

    # Assert that the fatal error was printed to the console
    captured = capsys.readouterr()
    assert f"Fatal error: {error_message}" in captured.out
    # Assert that the fatal error was logged
    assert f"Fatal error in calculator REPL: {error_message}" in caplog.text
